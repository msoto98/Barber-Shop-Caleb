  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <div className="text-[10px] uppercase" style={{ color: colors.muted, letterSpacing: "0.15em" }}>Reservas</div>
      </div>
    </div>
  );
}
...
function ClienteView({ bookings, blockedDays, blockedHours, onCreateBooking, onCancelBooking, onGoAdmin, heroImg }) {
  ...
  const waMessage = confirmed ? `Hola, soy ${confirmed.name}. Confirmo mi cita en Barber Shop Caleb: ${confirmed.serviceName} el ${confirmed.date} a las ${formatHour(confirmed.time)}.${confirmed.eyebrows ? " Con arreglo de cejas." : ""}` : "";

  return (
    <div className="max-w-md mx-auto">
      {step === 1 && view === "reservar" ? (
        <div className="relative rounded-2xl overflow-hidden mb-5" style={{ height: 190 }}>
          <img src={heroImg} alt="Barber Shop Caleb" className="w-full h-full object-cover" style={{ filter: "saturate(0.85) brightness(0.68)" }} />
